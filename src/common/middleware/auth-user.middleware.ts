...
export const authUser = async (request: Request, response: Response, next: NextFunction) => {
  const service = await ServiceController.findServiceById(request.params._id)
  if (!service) return response.status(404).json(new NotFoundError('Service', { name: '_id', value: request.params._id }).respond())
